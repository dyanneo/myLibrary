...
// Edit displays a book for a user to change
func Edit(w http.ResponseWriter, r *http.Request) {
	fmt.Println("top of Edit")
	db := dbConn()
	nID := r.URL.Query().Get("id")
	selDB, err := db.Query("SELECT * FROM book WHERE id=?", nID)
	if err != nil {
	...
	tmpl.ExecuteTemplate(w, "Edit", book)
	defer db.Close()
}

// Update applies changes to a book
func Update(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "POST" {
		...
		formattype := r.FormValue("formattype")
		location := r.FormValue("location")
		isbn := r.FormValue("isbnnumber")
		id := r.FormValue("id")
		insForm, err := db.Prepare("UPDATE Book SET title=?, author=?, formattype=?, location=?, isbn=? WHERE id=?")
		if err != nil {
			panic(err.Error())
		}
		insForm.Exec(title, author, formattype, location, isbn, id)
		log.Println("UPDATE: Title: " + title + " | Author: " + author + " | Format: " + formattype + " | Location: " + location + " | ISBN: " + isbn)
	}
	defer db.Close()
	http.Redirect(w, r, "/", 301)
}
